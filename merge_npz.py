...
for monkey in ['V', 'W']:
	for session in [0,1,2,3]:
		...
		# 	mat_value_dict[key] = np.concatenate(value)
		# scipy.io.savemat(f"data/spikes/monkey{monkey}_session{session}_spikes.mat", mat_spike_dict)  # shape = (time x neurons x trials) = (300 x 3000 x 1920)
		# scipy.io.savemat(f"data/spikes/monkey{monkey}_session{session}_values.mat", mat_value_dict)  # shape = (trials) = (1920)