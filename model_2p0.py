...
class Environment():
    ...
    def set_wt(self, sim, net):
        if self.reward == [1]:
            self.wt = 1 if sim.data[net.p_w][-100:][0].mean() > 0.5 else 0
        elif self.reward == [-1]:
            self.wt = 0 if sim.data[net.p_w][-100:][0].mean() > 0.5 else 1
    def sample_letter(self, t):
        return [1,0] if self.letter==[1] else [0,1]
    def sample_action(self, t):
        return self.action
    def sample_target(self, t):
        target = [0,0,0,0]
        target[0] = self.reward[0] if (self.action==[1] and self.letter==[1]) or (self.action==[-1] and self.letter==[-1]) else 0
        target[1] = self.reward[0] if (self.action==[1] and self.letter==[-1]) or (self.action==[-1] and self.letter==[1]) else 0
        target[2] = self.reward[0] if self.action==[1] else 0
        target[3] = self.reward[0] if self.action==[-1] else 0
        return target
    def sample_learning_rate(self, t):
        lr = [0,0,0,0]
        lr[0] = self.params['alpha_v'] if (self.action==[1] and self.letter==[1]) or (self.action==[-1] and self.letter==[-1]) else self.params['gamma_v']
        lr[1] = self.params['alpha_v'] if (self.action==[1] and self.letter==[-1]) or (self.action==[-1] and self.letter==[1]) else self.params['gamma_v']
        lr[2] = self.params['alpha_v'] if self.action==[1] else self.params['gamma_v']
        lr[3] = self.params['alpha_v'] if self.action==[-1] else self.params['gamma_v']
        return lr
    def sample_wt(self, t):
        return self.wt
    def sample_wlr(self, t):
        # return self.params['alpha_w'] if self.reward == [1] else self.params['gamma_w']
        return self.params['alpha_w']
...
def build_network(env, n_neurons=3000, seed_network=0, alpha_pes=3e-5):
    ...
    with net:
        ...
        in_target = nengo.Node(lambda t: env.sample_target(t))
        in_learning_rate = nengo.Node(lambda t: env.sample_learning_rate(t))
        in_wt = nengo.Node(lambda t: env.sample_wt(t))
        in_wlr = nengo.Node(lambda t: env.sample_wlr(t))
        ...
        ev = nengo.Ensemble(n_neurons, 8, radius=4)
        ew = nengo.Ensemble(n_neurons, 2, radius=2)
        vletout = nengo.Ensemble(1, 2, neuron_type=nengo.Direct())
        evout = nengo.Ensemble(1, 4, neuron_type=nengo.Direct())
        ewout = nengo.Ensemble(1, 1, neuron_type=nengo.Direct())
        ...
        cg = nengo.Connection(g, w, synapse=0.01, function=lambda x: 0.5, learning_rule_type=pes)
        ...
        nengo.Connection(v, ev[:4], synapse=0.01, transform=-1)  # minus value
        nengo.Connection(in_target, ev[:4])  # target
        nengo.Connection(in_learning_rate, ev[4:8])  # times chosen/unchosen learning rate
        nengo.Connection(in_phase[0], ev.neurons, transform=winh)  # turn off when not in learning phase

        nengo.Connection(w, ew[0], synapse=0.01, transform=-1)  # minus value
        nengo.Connection(in_wt, ew[0])  # target
        nengo.Connection(in_wlr, ew[1])  # times rewarded/unrewarded learning rate
        nengo.Connection(in_phase[0], ew.neurons, transform=winh)

        nengo.Connection(ev, cf.learning_rule, synapse=0.01, transform=-1, function=lambda x: [x[0]*x[4], x[1]*x[5], x[2]*x[6], x[3]*x[7]])  # error * learning rate
        nengo.Connection(ew, cg.learning_rule, synapse=0.01, transform=-1, function=lambda x: x[0]*x[1])  # error * learning rate
        ...
        nengo.Connection(ev, evout, synapse=0.01, function=lambda x: [x[0]*x[4], x[1]*x[5], x[2]*x[6], x[3]*x[7]])  # error * learning rate
        nengo.Connection(ew, ewout, synapse=0.01, function=lambda x: x[0]*x[1])  # error * learning rate
        ...
        net.p_ev = nengo.Probe(evout)
        net.p_ew = nengo.Probe(ewout)
...
def simulate_values_spikes(net):
    ...
    with sim:
        for trial in env.empirical.query("monkey==@monkey & session==@session & block==@block")['trial'].unique():
            ...
            t1 = t_end - 100
            ev = sim.data[net.p_ev][t1:t_end,:].mean(axis=0)
            ew = sim.data[net.p_ew][t1:t_end,0].mean()
...
if __name__ == "__main__":
    ...
    if param_config=='load':
        ...
        params = {
            ...
            # 'gamma_w':rng_network.uniform(0.1, 0.2),
        }
    s = time.time()
