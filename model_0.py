...
class Environment():
    def __init__(self, sid, seed=1, t_load=1, t_cue=0.5, t_reward=0.5, dt=0.001,
                 ...
                 omega_0=0.35, alpha_omega=0.31, gamma_omega=0.09):
        self.empirical = pd.read_pickle("data/empirical.pkl")
        self.sid = sid
        self.rng = np.random.RandomState(seed=seed)
        self.white_signal_left = nengo.processes.WhiteSignal(period=1000, high=noise_freq, rms=noise_rms, seed=seed)
        self.white_signal_right = nengo.processes.WhiteSignal(period=1000, high=noise_freq, rms=noise_rms, seed=seed+1)
        self.t_load = t_load
        self.t_cue = t_cue
...
def build_network(env, n_neurons=2000, seed_network=1, inh=0, k=1.0, a=4e-5):
    ...
    with net:
        ...
        # in_noise_action_left = nengo.Node(env.white_signal_left)
        # in_noise_action_right = nengo.Node(env.white_signal_right)
        ...
        # action_left = nengo.Ensemble(n_neurons, 1, encoders=nengo.dists.Choice([[1]]), intercepts=nengo.dists.Uniform(0,1))
        # action_right = nengo.Ensemble(n_neurons, 1, encoders=nengo.dists.Choice([[1]]), intercepts=nengo.dists.Uniform(0,1))
        ...
        # nengo.Connection(in_phase[2], load.neurons, transform=inh_feedback)
        ...
        # nengo.Connection(value_left, action_left, transform=k)
        # nengo.Connection(value_right, action_right, transform=k)
        # nengo.Connection(in_noise_action_left, action_left, synapse=None)
        # nengo.Connection(in_noise_action_right, action_right, synapse=None)
        # nengo.Connection(action_left, action_left, transform=1, synapse=0.1)
        # nengo.Connection(action_left, action_right, transform=inh, synapse=0.1)
        # nengo.Connection(action_right, action_left, transform=inh, synapse=0.1)
        # nengo.Connection(action_right, action_right, transform=1, synapse=0.1)
        ...
        # nengo.Connection(in_phase[1], action_left.neurons, transform=inh_feedback, synapse=None)
        # nengo.Connection(in_phase[1], action_right.neurons, transform=inh_feedback, synapse=None)
        # nengo.Connection(in_phase[2], action_left.neurons, transform=inh_feedback, synapse=None)
        # nengo.Connection(in_phase[2], action_right.neurons, transform=inh_feedback, synapse=None)
        ...
        # net.p_action_left = nengo.Probe(action_left)
        # net.p_action_right = nengo.Probe(action_right)
